...
//    static final char[] KEYSTORE_PASSWORD = "KeyStore_Password".toCharArray();
    ...
    private void createIdentity(KeyStore keyStore, String str, String str2) throws GeneralSecurityException {
        ...
        try {
            keyStore.setKeyEntry(str, generateKeyPair.getPrivate(), androidRemoteContext.getKeyStorePass(), new Certificate[]{SslUtil.generateX509V3Certificate(generateKeyPair, getCertificateName(str2))});
    ...
    private KeyStore load() {
        ...
        try {
            if (!useAndroidKeyStore()) {
                keyStore = KeyStore.getInstance(KeyStore.getDefaultType());
                keyStore.load(new java.io.FileInputStream(androidRemoteContext.getKeyStoreFile()), androidRemoteContext.getKeyStorePass());
            } else {
                keyStore = KeyStore.getInstance(ANDROID_KEYSTORE);
                keyStore.load(null);
            }
    ...
    public KeyManager[] getKeyManagers() throws GeneralSecurityException {
        ...
        keyManagerFactory.init(this.mKeyStore, androidRemoteContext.getKeyStorePass());
        KeyManager[] managers = keyManagerFactory.getKeyManagers();
        for (int i = 0; i < managers.length; i++) {
            if (managers[i] instanceof javax.net.ssl.X509KeyManager) {
                final javax.net.ssl.X509KeyManager defaultKeyManager = (javax.net.ssl.X509KeyManager) managers[i];
                managers[i] = new javax.net.ssl.X509KeyManager() {
                    @Override
                    public String chooseClientAlias(String[] keyType, java.security.Principal[] issuers, java.net.Socket socket) {
                        try {
                            return mKeyStore.aliases().nextElement();
                        } catch (Exception e) {
                            return SERVER_IDENTITY_ALIAS;
                        }
                    }
                    @Override
